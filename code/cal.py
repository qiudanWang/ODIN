...
def test(nnName, dataName, CUDA_DEVICE, epsilon, temperature):
    ...
    net1 = torch.load("../models/{}.pth".format(nnName), map_location=torch.device('cpu'))
    ...
    subset_indices = [i for i in range(0, 2)]
    subset = torch.utils.data.Subset(testset, subset_indices)
    testloaderIn = torch.utils.data.DataLoader(subset, batch_size=1, shuffle=False, num_workers=2)
